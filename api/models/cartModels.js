...
const getCart = (id, result,) => {
  db.query("SELECT Cart.cartID, Cart.createdAt, Products.prodName AS prodName, Products.amount, Products.quantity, Products.prodUrl FROM Cart JOIN Products ON Cart.prodID = Products.prodID WHERE Cart.userID = ?",
   [id], (err, results) => {             
      if(err) {
          console.log(err);
          result(err, null);
